...
def infer_from_nodePD(nodePD, driving_scaler_path=None, road_scaler_path=None, model_path=None,
                      past_steps=50, future_steps=100, steps_to_predict_at_a_time=10,
                      filter_on=True, tau_plot=0.7, dt_plot=0.1,
                      driving_scaler=None, road_scaler=None, model=None, batch_size=128):
    ...
    # allow callers to pass preloaded scalers/models to avoid repeated loads
    if driving_scaler is None:
        if driving_scaler_path is None:
            raise ValueError('driving_scaler or driving_scaler_path must be provided')
        driving_scaler = joblib.load(driving_scaler_path)
    if road_scaler is None:
        if road_scaler_path is None:
            raise ValueError('road_scaler or road_scaler_path must be provided')
    ...
    # convert relevant columns to numpy arrays once (faster than repeated iloc)
    padded_driving = padded[DRIVING_FEATURES].to_numpy()
    padded_road = padded[ROAD_FEATURES].to_numpy()

    prediction_horizon = len(raw_df)

    # Build list of window start indices for batch prediction (advance by steps_to_predict_at_a_time)
    starts = []
    idx = past_steps
    while len(starts) * steps_to_predict_at_a_time < prediction_horizon:
        future_end = idx + future_steps
        if future_end > len(padded_driving):
            break
        starts.append(idx)
        idx += steps_to_predict_at_a_time

    if not starts:
        # nothing to predict
        horizons_scaled = []
        horizons_starts = []
        all_pred_scaled = np.array([])
    else:
        num_iters = len(starts)
        # allocate arrays for batch inputs
        Xd_batch = np.zeros((num_iters, past_steps, len(DRIVING_FEATURES)), dtype=np.float32)
        Xrp_batch = np.zeros((num_iters, past_steps, len(ROAD_FEATURES)), dtype=np.float32)
        Xrf_batch = np.zeros((num_iters, future_steps, len(ROAD_FEATURES)), dtype=np.float32)
        horizons_starts = []
        for i, s in enumerate(starts):
            Xd_batch[i] = padded_driving[s - past_steps: s]
            Xrp_batch[i] = padded_road[s - past_steps: s]
            Xrf_batch[i] = padded_road[s: s + future_steps]
            horizons_starts.append(s - past_steps)

        # predict in batch
        batch_pred = model.predict([Xd_batch, Xrp_batch, Xrf_batch], batch_size=batch_size, verbose=0)
        horizons_scaled = [batch_pred[i].copy() for i in range(batch_pred.shape[0])]

        # aggregate first steps_to_predict_at_a_time from each horizon in order
        collected = []
        for i in range(batch_pred.shape[0]):
            h = batch_pred[i]
            collected.extend(h[:steps_to_predict_at_a_time].tolist())
        all_pred_scaled = np.array(collected)
    ...
    # Simplified zeroing rule: if the (simulated) target speed is < 5 m/s
    ...
    for i in range(1, n_pred):
        if np.isnan(pred_full[i]):
            continue
        if pred_full[i] < 4.0 and (target_speed_arr[i] < 3.0):
            pred_full[i] = 0.0
...
def plot_predictions(out_df, horizons_unscaled=None, horizons_starts=None, az_change_th=30.0, seq_alpha=0.08):
    ...
    # convert speeds to mph for plotting
    mph = 2.2369362920544
    ...
    # Plot predicted speed (note: may already be filtered by infer_from_nodePD) in mph
